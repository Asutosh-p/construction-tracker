...
/**
 * Auto-generated WorkflowTemplate Routes
 *
...
import type { WorkflowTemplateHooks } from "./workflow-template.hooks"
...
let hooks: WorkflowTemplateHooks = {}
...
/**
 * Set hooks for WorkflowTemplate routes
 */
export function setWorkflowTemplateHooks(h: WorkflowTemplateHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "itemTypeId", "description", "isDefault", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["steps"] as const
...
/**
 * Create WorkflowTemplate CRUD routes
 */
export function createWorkflowTemplateRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[WorkflowTemplate] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.workflowTemplate.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "WorkflowTemplate not found" } }, 404)
      }
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.workflowTemplate.create({
        data: picked.data,
      })
      ...
      console.error("[WorkflowTemplate] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.workflowTemplate.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[WorkflowTemplate] Update error:", error)
