...
/**
 * Auto-generated RoomItemTemplate Routes
 *
...
import type { RoomItemTemplateHooks } from "./room-item-template.hooks"
...
let hooks: RoomItemTemplateHooks = {}
...
/**
 * Set hooks for RoomItemTemplate routes
 */
export function setRoomItemTemplateHooks(h: RoomItemTemplateHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["buildingTypeId", "roomName", "itemTypeId", "code", "name", "sortOrder", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["buildingType", "itemType"] as const
...
/**
 * Create RoomItemTemplate CRUD routes
 */
export function createRoomItemTemplateRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        prisma.roomItemTemplate.findMany({
          where,
          include,
          orderBy,
        ...
        prisma.roomItemTemplate.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[RoomItemTemplate] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.roomItemTemplate.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "RoomItemTemplate not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[RoomItemTemplate] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.roomItemTemplate.create({
        data: picked.data,
      })
      ...
      console.error("[RoomItemTemplate] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.roomItemTemplate.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[RoomItemTemplate] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await prisma.roomItemTemplate.delete({
        where: { id },
      })
      ...
      console.error("[RoomItemTemplate] Delete error:", error)
