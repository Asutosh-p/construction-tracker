...
/**
 * Auto-generated WorkflowTemplateStep Routes
 *
...
import type { WorkflowTemplateStepHooks } from "./workflow-template-step.hooks"
...
let hooks: WorkflowTemplateStepHooks = {}
...
/**
 * Set hooks for WorkflowTemplateStep routes
 */
export function setWorkflowTemplateStepHooks(h: WorkflowTemplateStepHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "sortOrder", "templateId", "createdAt"] as const
const RELATION_FIELDS = ["template"] as const
...
/**
 * Create WorkflowTemplateStep CRUD routes
 */
export function createWorkflowTemplateStepRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.workflowTemplateStep.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[WorkflowTemplateStep] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.workflowTemplateStep.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "WorkflowTemplateStep not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[WorkflowTemplateStep] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.workflowTemplateStep.create({
        data: picked.data,
      })
      ...
      console.error("[WorkflowTemplateStep] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.workflowTemplateStep.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[WorkflowTemplateStep] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await prisma.workflowTemplateStep.delete({
        where: { id },
      })
      ...
      console.error("[WorkflowTemplateStep] Delete error:", error)
