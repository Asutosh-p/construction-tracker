...
/**
 * Auto-generated ItemType Routes
 *
...
import type { ItemTypeHooks } from "./item-type.hooks"
...
/**
 * Set hooks for ItemType routes
 */
export function setItemTypeHooks(h: ItemTypeHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["code", "name", "description", "isDefault", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["items", "roomTemplates"] as const
...
/**
 * Create ItemType CRUD routes
 */
export function createItemTypeRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[ItemType] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "ItemType not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[ItemType] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      console.error("[ItemType] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.itemType.update({
        where: { id },
        data: picked.data,
      })
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[ItemType] Delete error:", error)
