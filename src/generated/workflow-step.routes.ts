...
/**
 * Auto-generated WorkflowStep Routes
 *
...
import type { WorkflowStepHooks } from "./workflow-step.hooks"
...
/**
 * Set hooks for WorkflowStep routes
 */
export function setWorkflowStepHooks(h: WorkflowStepHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "sortOrder", "status", "percent", "notes", "dateCompleted", "itemId", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["item"] as const
...
/**
 * Create WorkflowStep CRUD routes
 */
export function createWorkflowStepRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[WorkflowStep] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.workflowStep.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "WorkflowStep not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[WorkflowStep] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.workflowStep.create({
        data: picked.data,
      })
      ...
      console.error("[WorkflowStep] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.workflowStep.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[WorkflowStep] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[WorkflowStep] Delete error:", error)
