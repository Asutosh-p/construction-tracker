...
import type { FloorHooks } from "./floor.hooks"
...
/**
 * Set hooks for Floor routes
 */
export function setFloorHooks(h: FloorHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "buildingId", "sortOrder", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["building", "rooms", "installationItems"] as const
...
/**
 * Create Floor CRUD routes
 */
export function createFloorRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[Floor] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Floor not found" } }, 404)
      }
