...
import type { RoomHooks } from "./room.hooks"
...
/**
 * Set hooks for Room routes
 */
export function setRoomHooks(h: RoomHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "floorId", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["floor", "items"] as const
...
/**
 * Create Room CRUD routes
 */
export function createRoomRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Room not found" } }, 404)
      }
