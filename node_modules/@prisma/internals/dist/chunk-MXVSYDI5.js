...
var chunk_MXVSYDI5_exports = {};
__export(chunk_MXVSYDI5_exports, {
...
module.exports = __toCommonJS(chunk_MXVSYDI5_exports);
...
async function formatSchema({ schemas }, inputFormattingOptions) {
  if (process.env.FORCE_PANIC_PRISMA_SCHEMA) {
    handleFormatPanic(() => {
      import_chunk_KZULYQKN.prismaSchemaWasm.debug_panic();
    });
  }
  ...
  const { formattedMultipleSchemas, lintDiagnostics } = handleFormatPanic(() => {
    ...
    const lintDiagnostics2 = (0, import_chunk_LFP3WIF2.lintSchema)({ schemas: formattedMultipleSchemas2 });
    return { formattedMultipleSchemas: formattedMultipleSchemas2, lintDiagnostics: lintDiagnostics2 };
  });
  const lintWarnings = (0, import_chunk_LFP3WIF2.getLintWarningsAsText)(lintDiagnostics);
...
function formatWasm(schema, documentFormattingParams) {
  const formattedSchema = import_chunk_KZULYQKN.prismaSchemaWasm.format(schema, JSON.stringify(documentFormattingParams));
...
async function processSchemaResult({
  ...
  const configFromPsl = await (0, import_chunk_4EXU7SPB.getConfig)({ datamodel: schemaResult.schemas });
...
async function getGenerators(options) {
  ...
  const dmmf = await (0, import_chunk_HSVC4GON.getDMMF)({
...
You need to define \`output\` in the generator block in the schema file.`
        ...
        const datamodel = (0, import_chunk_K3HDOXKJ.mergeSchemas)({ schemas: schemaContext.schemaFiles });
        const options2 = {
          datamodel,
