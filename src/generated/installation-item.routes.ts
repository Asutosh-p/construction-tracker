...
/**
 * Auto-generated InstallationItem Routes
 *
...
import type { InstallationItemHooks } from "./installation-item.hooks"
...
let hooks: InstallationItemHooks = {}
...
/**
 * Set hooks for InstallationItem routes
 */
export function setInstallationItemHooks(h: InstallationItemHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["code", "name", "roomId", "floorId", "itemTypeId", "level", "status", "percent", "notes", "isVariation", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["room", "floor", "itemType", "workflowSteps"] as const
...
/**
 * Create InstallationItem CRUD routes
 */
export function createInstallationItemRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.installationItem.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[InstallationItem] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "InstallationItem not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[InstallationItem] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      console.error("[InstallationItem] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.installationItem.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[InstallationItem] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await prisma.installationItem.delete({
        where: { id },
      })
      ...
      console.error("[InstallationItem] Delete error:", error)
