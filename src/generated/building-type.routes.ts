...
/**
 * Auto-generated BuildingType Routes
 *
...
import type { BuildingTypeHooks } from "./building-type.hooks"
...
/**
 * Set hooks for BuildingType routes
 */
export function setBuildingTypeHooks(h: BuildingTypeHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["code", "name", "bedrooms", "hasWellness", "hasLobby", "description", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["buildings", "roomTemplates"] as const
...
/**
 * Create BuildingType CRUD routes
 */
export function createBuildingTypeRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[BuildingType] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.buildingType.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "BuildingType not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[BuildingType] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.buildingType.create({
        data: picked.data,
      })
      ...
      console.error("[BuildingType] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.buildingType.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[BuildingType] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[BuildingType] Delete error:", error)
